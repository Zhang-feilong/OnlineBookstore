  ...
  <head>
	...
	<script type="text/javascript">
		...
		$(":checkbox[name=checkboxBtn]").click(function(){
			...
			var select = $(":checkbox[name=checkboxBtn][checked=true]").length;//被选中所有条目数
			if(all == select){
		...
		$(".jian").click(function(){
			...
			if(quantity==1){
				if(confirm("您是否要删除该条目？")){
					location = "/shop/CartItemServlet?method=batchDelete&cartItemIds=" + id;
				}
			}else{
		...
		$(":checkbox[name=checkboxBtn][checked=true]").each(function(){
		 //获取复选框的值，即cartItemId,再通过它找到小计，获取其文本
		 var id =$(this).val();
		 var text = $("#" + id + "Subtotal").text();
		 total +=Number(text);
		 }); 
		 $("#total").text(round(total,2));//round函数保留两位小数
		}
		...
		function batchDelete(){
			...
			3.指定location为CartItemServlet,参数为method=batchDelete,参数cartItemIds=数组的toString()
			*/
			var cartItemIdArray = new Array();
			$(":checkbox[name=checkboxBtn][checked=true]").each(function(){
				cartItemIdArray.push($(this).val());
			});
			location = "/shop/CartItemServlet?method=batchDelete&cartItemIds=" + cartItemIdArray;
